...
pub async fn init_application() -> Result<()> {
    ...
    let current_dir = env::current_dir()
        .map_err(|e| crate::error::Error::Filesystem(format!("Failed to get current directory: {}", e)))?;

    let root_path = find_project_root(current_dir)
        .ok_or_else(|| crate::error::Error::Filesystem("Failed to find project root".to_string()))?;
    ...
    if rushd_config_path.exists() {
        ...
        let mut file = File::open(&rushd_config_path)
            .map_err(|e| crate::error::Error::Filesystem(format!("Failed to open rushd.yaml: {}", e)))?;

        let mut contents = String::new();
        file.read_to_string(&mut contents)
            .map_err(|e| crate::error::Error::Filesystem(format!("Failed to read rushd.yaml: {}", e)))?;

        let rushd_config: RushdConfig = serde_yaml::from_str(&contents)
            .map_err(|e| crate::error::Error::Config(format!("Failed to parse rushd.yaml: {}", e)))?;
