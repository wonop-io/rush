...
///
/// Returns a HashMap containing all key-value pairs found in the file
pub fn load_dotenv(path: &Path) -> Result<HashMap<String, String>, std::io::Error> {
    ...
    while let Some(Ok(line)) = lines_iter.next() {
        ...
        if let Some((key, value)) = line.split_once('=') {
            ...
            let processed_value = process_env_value(&value_str, &mut lines_iter);
            env_map.insert(key, processed_value);
        }
    }
...
/// Process an environment variable value, handling quoted and multi-line values
fn process_env_value(
    value_str: &str,
    lines_iter: &mut std::iter::Peekable<std::io::Lines<BufReader<File>>>,
) -> String {
...
///
/// Takes a HashMap of key-value pairs and writes them to the specified file
