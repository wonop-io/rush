...
pub async fn execute(
    ...
    println!("\n{} {} to {}", 
             ...
             std::env::var("RUSH_ENV").unwrap_or_else(|_| "default".to_string()).yellow());
    
    if deployment_config.dry_run {
        println!("{} {}", "Mode:".blue(), "DRY RUN".yellow().bold());
    }
    
    println!("{} {}\n", "Strategy:".blue(), format!("{:?}", deployment_config.strategy).white());
    ...
    reporter.start_step(&format!("Applying {:?} deployment", deployment_config.strategy));
    ...
    let deployment_result = deploy_to_kubernetes(
        config.clone(),
        deployment_config.dry_run,
    ).await;
...
fn validate_environment(config: &Config) -> Result<()> {
    ...
    if kubectl_check.is_err() {
        return Err(Error::External("kubectl not found. Please install kubectl.".to_string()));
    }
    ...
    if docker_check.is_err() {
        return Err(Error::External("docker not found. Please install docker.".to_string()));
    }
...
async fn create_reactor(config: Arc<Config>) -> Result<Arc<tokio::sync::Mutex<rush_container::reactor::Reactor>>> {
    ...
    let network_manager = Arc::new(rush_container::network::NetworkManager::new(
        docker_client.clone(),
        &config.product_name(),
    ).await?);
...
async fn apply_deployment_strategy(
    ...
    match strategy {
        DeploymentStrategy::RollingUpdate { max_surge, max_unavailable } => {
            info!("Configuring rolling update (max_surge: {}, max_unavailable: {})", 
...
async fn verify_deployment(
    ...
    info!("Waiting for deployments to be ready (timeout: {}s)", timeout);
    ...
    Err(Error::Deploy("Deployment verification timed out".to_string()))
}
...
pub async fn get_status(config: Arc<Config>) -> Result<()> {
    ...
    let deployments = kubectl.execute(vec![
        "get".to_string(),
    ...
    let services = kubectl.execute(vec![
        "get".to_string(),
        "services".to_string(),
        "-o".to_string(),
        "wide".to_string(),
