...
pub fn parse_args() -> clap::ArgMatches {
    ...
    Command::new("rush")
        ...
        .arg(arg!(skip_checks: --"skip-checks" "Skip preflight dependency checks"))
        .arg(Arg::new("product_name")
            .help("Product name (e.g., helloworld.wonop.io)")
            .index(1))
        ...
        .subcommand(Command::new("dev")
            ...
            .arg(arg!(--"log-level" <LEVEL> "Minimum log level to display: trace, debug, info, warn, error").default_value("info"))
            ...
            .arg(arg!(--"filter-components" <COMPONENTS> ... "Show only these components").num_args(1..))
            .arg(arg!(--"exclude-components" <COMPONENTS> ... "Hide these components").num_args(1..))
            .arg(arg!(--"show-build-only" "Show only build/compilation output"))
            .arg(arg!(--"show-runtime-only" "Show only runtime output"))
        )
        .subcommand(Command::new("build")
            .about("Build Docker images for all components")
            .arg(arg!(--"force-rebuild" "Force rebuild of all components, ignoring cache")))
        ...
        .subcommand(Command::new("deploy")
            .about("Deploy the product to Kubernetes")
            .arg(arg!(--"dry-run" "Perform a dry run without making changes"))
            .arg(arg!(--"force-rebuild" "Force rebuild of all images"))
            .arg(arg!(--"skip-push" "Skip pushing images to registry"))
            .arg(arg!(--"no-wait" "Don't wait for deployments to be ready"))
            .arg(arg!(--"no-rollback" "Disable automatic rollback on failure"))
            .arg(arg!(--strategy <STRATEGY> "Deployment strategy: rolling, blue-green, canary, direct"))
            .arg(arg!(--timeout <SECONDS> "Timeout for deployment verification"))
        )
        .subcommand(Command::new("install"))
        .subcommand(Command::new("uninstall"))
        .subcommand(Command::new("apply")
            .about("Apply Kubernetes manifests to the cluster")
            .arg(arg!(--"dry-run" "Perform a dry run without making changes"))
        )
        .subcommand(Command::new("unapply")
            .about("Remove Kubernetes resources from the cluster")
            .arg(arg!(--"dry-run" "Perform a dry run without making changes"))
        )
        ...
        .subcommand(Command::new("vault")
            ...
            .subcommand(Command::new("migrate")
                .about("Migrates secrets from one vault to another")
                .long_about("Migrates secrets between vault backends.\n\nExamples:\n  rush <product> vault migrate json:~/.rush/vaults  # Migrate to JSON vault\n  rush <product> vault migrate .env                   # Migrate to .env files")
                .arg(Arg::new("dest")
                    .required(true)
                    .help("Destination vault (e.g., 'json:/path/to/vault' or '.env')"))
                .arg(Arg::new("from")
                    .long("from")
                    .help("Source vault type (defaults to current vault). Options: .env, json, 1Password"))
            )
        )
        ...
        .subcommand(Command::new("profile")
            .about("Profile Rush performance")
            .arg(arg!(--"output" <FILE> "Output file for profile data").default_value("rush-profile.json"))
            .arg(arg!(--"format" <FORMAT> "Output format: json, flamegraph, chrome-trace").default_value("json"))
            .arg(arg!(<command> "Command to profile (build, dev, push)").required(true))
            .arg(arg!([args]... "Additional arguments to pass to the profiled command"))
