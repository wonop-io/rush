...
pub async fn start_local_services(
    ...
    info!(
        "Local services started successfully with {} environment variables",
        all_env_vars.len()
    );
...
fn register_local_service(
    ...
    } = &spec.build_type
    {
        info!(
            ...
            spec.component_name, service_type
        );
...
fn register_stripe_service(
    ...
    let service = ProcessLocalService::stripe_cli(spec.component_name.clone(), webhook_url);
