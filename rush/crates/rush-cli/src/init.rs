...
use rush_utils::version::check_version;
use rush_utils::{find_project_root, Directory};
...
pub async fn init_application() -> Result<()> {
    ...
    if rushd_config_path.exists() {
        ...
        let mut file = File::open(&rushd_config_path).map_err(|e| {
            rush_core::error::Error::Filesystem(format!("Failed to open rushd.yaml: {e}"))
        })?;

        let mut contents = String::new();
        file.read_to_string(&mut contents).map_err(|e| {
            rush_core::error::Error::Filesystem(format!("Failed to read rushd.yaml: {e}"))
        })?;

        let rushd_config: RushdConfig = serde_yaml::from_str(&contents).map_err(|e| {
            rush_core::error::Error::Config(format!("Failed to parse rushd.yaml: {e}"))
        })?;
