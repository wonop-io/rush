...
fn test_config_with_custom_settings() {
    ...
    let mut config = LocalServiceConfig::new("postgres".to_string(), LocalServiceType::PostgreSQL);
...
fn test_get_container_name() {
    let config = LocalServiceConfig::new("my-postgres".to_string(), LocalServiceType::PostgreSQL);
...
fn test_redis_config() {
    let config = LocalServiceConfig::new("redis".to_string(), LocalServiceType::Redis);
...
fn test_minio_config() {
    let mut config = LocalServiceConfig::new("minio".to_string(), LocalServiceType::MinIO);

    config
        .env
        .insert("MINIO_ROOT_USER".to_string(), "minioadmin".to_string());
    config
        .env
        .insert("MINIO_ROOT_PASSWORD".to_string(), "minioadmin".to_string());

    assert_eq!(config.get_image(), "minio/minio:latest");
    assert_eq!(
        config.env.get("MINIO_ROOT_USER"),
