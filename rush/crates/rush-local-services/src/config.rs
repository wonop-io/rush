use crate::types::{LocalServiceType, PortMapping, VolumeMapping, ResourceLimits};
    ...
    pub fn with_defaults(mut self) -> Self {
        ...
        if self.ports.is_empty() {
            if let Some(default_port) = self.service_type.default_port() {
                self.ports.push(PortMapping::new(default_port, default_port));
            }
        }
        ...
        match &self.service_type {
            LocalServiceType::PostgreSQL => {
                self.env.entry("POSTGRES_DB".to_string())
                    .or_insert("postgres".to_string());
                self.env.entry("POSTGRES_USER".to_string())
                    .or_insert("postgres".to_string());
                self.env.entry("POSTGRES_PASSWORD".to_string())
                    .or_insert("postgres".to_string());
            },
            LocalServiceType::MySQL => {
                self.env.entry("MYSQL_ROOT_PASSWORD".to_string())
                    .or_insert("root".to_string());
                self.env.entry("MYSQL_DATABASE".to_string())
                    .or_insert("mysql".to_string());
            },
            LocalServiceType::MongoDB => {
                self.env.entry("MONGO_INITDB_ROOT_USERNAME".to_string())
                    .or_insert("root".to_string());
                self.env.entry("MONGO_INITDB_ROOT_PASSWORD".to_string())
                    .or_insert("root".to_string());
            },
            LocalServiceType::LocalStack => {
                self.env.entry("SERVICES".to_string())
                    .or_insert("s3,sqs,sns,dynamodb".to_string());
                self.env.entry("AWS_DEFAULT_REGION".to_string())
                    .or_insert("us-east-1".to_string());
                self.env.entry("EDGE_PORT".to_string())
                    .or_insert("4566".to_string());
            },
            LocalServiceType::MinIO => {
                self.env.entry("MINIO_ROOT_USER".to_string())
                    .or_insert("minioadmin".to_string());
                self.env.entry("MINIO_ROOT_PASSWORD".to_string())
                    .or_insert("minioadmin".to_string());
...
impl ServiceDefaults {
    pub fn postgres(name: String) -> LocalServiceConfig {
        LocalServiceConfig::new(name, LocalServiceType::PostgreSQL)
            .with_defaults()
    }
    
    pub fn redis(name: String) -> LocalServiceConfig {
        LocalServiceConfig::new(name, LocalServiceType::Redis)
            .with_defaults()
    }
    
    pub fn minio(name: String) -> LocalServiceConfig {
        LocalServiceConfig::new(name, LocalServiceType::MinIO)
            .with_defaults()
    }
