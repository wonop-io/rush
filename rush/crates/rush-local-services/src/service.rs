...
impl LocalServiceHandle {
    ...
    pub fn new(
        ...
        let health_check = config.get_health_check()
            .map(|cmd| HealthCheck::new(cmd));
    ...
    pub fn is_running(&self) -> bool {
        matches!(self.status, ServiceStatus::Running | ServiceStatus::Unhealthy(_))
    }
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        if let Ok(existing) = self.docker_client.get_container_by_name(&container_name).await {
            info!("Found existing container for {}, removing it", self.config.name);
    ...
    pub async fn stop(&mut self) -> Result<()> {
        if let Some(container_id) = &self.container_id {
            ...
            self.docker_client.stop_container(container_id).await
                .map_err(|e| Error::Docker(format!("Failed to stop {}: {}", self.config.name, e)))?;
            
            // Only remove if not persisting data
            if !self.config.persist_data {
                self.docker_client.remove_container(container_id).await
                    .map_err(|e| Error::Docker(format!("Failed to remove {}: {}", self.config.name, e)))?;
            }
        ...
        if let Some(container_id) = &self.container_id {
            ...
            match self.docker_client.container_status(container_id).await {
                Ok(ContainerStatus::Running) => {
                    ...
                    if let Some(health_check) = &self.health_check {
                        let result = self.docker_client
                            .exec_in_container(container_id, &health_check.command.split_whitespace().collect::<Vec<_>>())
                            .await;
                ...
                Ok(ContainerStatus::Exited(code)) => {
                    self.status = ServiceStatus::Failed(format!("Container exited with code {}", code));
            ...
            loop {
                match self.check_health().await? {
                    ...
                    _ => {
                        ...
                        debug!("Health check attempt {}/{} for {}", 
    ...
    pub async fn logs(&self, lines: usize) -> Result<String> {
        if let Some(container_id) = &self.container_id {
            self.docker_client.container_logs(container_id, lines).await
                .map_err(|e| Error::Docker(format!("Failed to get logs for {}: {}", self.config.name, e)))
        } else {
            Ok(String::new())
        }
    }
