...
impl DockerLocalService {
    ...
    async fn follow_logs_to_output(container_id: String, output: ServiceOutput) {
...
impl LocalService for DockerLocalService {
    async fn start(&mut self) -> Result<()> {
        self.output
            .info(format!("Starting Docker local service: {}", self.name))
            .await;
        ...
        if self.service_type == LocalServiceType::PostgreSQL
            && !self.config.env.contains_key("POSTGRES_HOST_AUTH_METHOD")
        {
        ...
        self.output
            .info(format!("Pulling image {image} if needed"))
            .await;
        ...
        self.output
            .info(format!(
                "Docker local service {} started successfully",
                self.name
            ))
            .await;
        ...
        if self.output.has_sink() {
            ...
            tokio::spawn(async move {
                Self::follow_logs_to_output(container_id_clone, output_clone).await;
            });
        }
        ...
        if !self.config.init_scripts.is_empty() {
            self.output
                .info(format!("Running initialization scripts for {}", self.name))
                .await;
            for script in &self.config.init_scripts {
                self.output
                    .info(format!("Running init script: {script}"))
                    .await;
    ...
    async fn stop(&mut self) -> Result<()> {
        self.output
            .info(format!("Stopping Docker local service: {}", self.name))
            .await;
            ...
            .await
        {
            Ok(existing_id) => {
                self.output
                    .info(format!("Stopping container {container_name} by name"))
                    .await;

                // Stop the container
                if let Err(e) = self.docker_client.stop_container(&existing_id).await {
                    self.output
                        .error(format!("Failed to stop container {container_name}: {e}"))
                        .await;
                }
                ...
                self.output
                    .info(format!("Removing container {container_name}"))
                    .await;
                if let Err(e) = self.docker_client.remove_container(&existing_id).await {
                    self.output
                        .error(format!("Failed to remove container {container_name}: {e}"))
                        .await;
                }
            }
            Err(_) => {
                ...
                self.output
                    .info(format!(
                        "Container {container_name} not found or already stopped"
                    ))
                    .await;
            }
        }
        ...
        self.output
            .info(format!("Docker local service {} stopped", self.name))
            .await;
    ...
    async fn run_post_startup_tasks(&mut self) -> Result<()> {
        ...
        self.output
            .info(format!("Running post-startup tasks for {}", self.name))
            .await;
        ...
        for task in &self.config.post_startup_tasks.clone() {
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    self.output
                        .info(format!("  ⎿  Task failed (may be expected): {e}"))
                        .await;
                }
            }
        }
        ...
        self.output
            .info(format!("Post-startup tasks completed for {}", self.name))
            .await;
        Ok(())
    }
