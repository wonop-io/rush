...
impl LocalServiceManager {
    ...
    fn start_internal<'a>(&'a self, name: &'a str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + 'a>> {
        Box::pin(async move {
            self.start_impl(name).await
        })
    }
    ...
    async fn start_impl(&self, name: &str) -> Result<()> {
        ...
        let mut service = LocalServiceHandle::new(
            config.clone(),
    ...
    fn stop_internal<'a>(&'a self, name: &'a str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<()>> + Send + 'a>> {
        Box::pin(async move {
            self.stop_impl(name).await
        })
    }
            ...
            match &config.service_type {
                ...
                crate::types::LocalServiceType::LocalStack => {
                    ...
                    connections.insert(
                        ...
                        config.env.get("AWS_DEFAULT_REGION").unwrap_or(&default_region).clone()
                    );
                },
                _ => {}
            }
    ...
    fn visit_dependencies(
        ...
        if let Some(&in_progress) = visited.get(name) {
            if in_progress {
                return Err(Error::Configuration(format!("Circular dependency detected: {}", name)));
            }
            return Ok(());
        }
    ...
    async fn run_init_scripts(&self, service: &LocalServiceHandle, config: &LocalServiceConfig) -> Result<()> {
