...
impl LocalServiceManager {
    ...
    pub fn register(&mut self, mut service: Box<dyn LocalService>) {
        ...
        if tokio::runtime::Handle::try_current().is_ok() {
            ...
            tokio::spawn(async move {
                output
                    .info(format!("Registering local service: {name}"))
                    .await;
            });
        }
    ...
    pub async fn wait_for_healthy(&mut self, timeout_duration: Duration) -> Result<()> {
        self.output
            .info(format!(
                "Waiting for all services to be healthy (timeout: {timeout_duration:?})"
            ))
            .await;
        ...
        for service in &self.services {
            ...
            self.output
                .info(format!("Waiting for {name} to be healthy..."))
                .await;
            ...
            let result = timeout(remaining, async {
                loop {
                    match service.is_healthy().await {
                        Ok(true) => {
                            self.output.info(format!("{name} is healthy")).await;
                            return Ok::<(), Error>(());
                        }
                        Ok(false) => {
            ...
            match result {
                ...
                Err(_) => {
                    return Err(Error::Config(format!(
                        "Timeout waiting for {name} to be healthy"
                    )));
                }
            }
        }
        ...
        self.output
            .info("Running post-startup tasks for services")
            .await;
        for service in &mut self.services {
