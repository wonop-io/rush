    ...
    async fn ensure_network_exists(&mut self) -> Result<()> {
        ...
        match self.docker_client.network_exists(&self.network_name).await {
            ...
            Ok(false) => {
                ...
                self.docker_client.create_network(&self.network_name).await?;
                self.created = true;
...
impl Drop for NetworkManager {
    fn drop(&mut self) {
        if self.created {
            debug!("NetworkManager dropping, will clean up network: {}", self.network_name);
...
mod tests {
    ...
    fn test_compute_network_name() {
        assert_eq!(NetworkManager::compute_network_name("helloworld.wonop.io"), "net-helloworld-wonop-io");
        assert_eq!(NetworkManager::compute_network_name("simple"), "net-simple");
        assert_eq!(NetworkManager::compute_network_name("complex.multi.dot"), "net-complex-multi-dot");
    }
}