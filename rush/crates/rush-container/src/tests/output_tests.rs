...
async fn test_follow_logs_from_start_command() -> Result<()> {
    ...
    for arg in &expected_args {
        assert!(
            ...
            "Argument {} should be included in docker logs command", arg
        );
    }
...
async fn test_startup_logs_not_missed() -> Result<()> {
    ...
    docker_client.add_container_logs(&container_id, early_startup_logs.clone()).await;
    ...
    for log in &early_startup_logs {
        assert!(
            logs.contains(log),
            "Startup log '{}' should be captured", log
        );
    }
...
async fn test_multi_container_log_separation() -> Result<()> {
    ...
    let containers = vec![
        ("frontend", docker_client.run_container("fe:1", "frontend", "net", &[], &[], &[]).await?),
        ("backend", docker_client.run_container("be:1", "backend", "net", &[], &[], &[]).await?),
        ("database", docker_client.run_container("db:1", "database", "net", &[], &[], &[]).await?),
    ];
    
    // Add unique logs for each container
    for (name, id) in &containers {
        docker_client.add_container_logs(
            id,
...
async fn test_continuous_log_streaming() -> Result<()> {
    ...
    for batch in &log_batches {
        docker_client.add_container_logs(&container_id, batch.clone()).await;
    }
...
async fn test_build_vs_runtime_phase_distinction() -> Result<()> {
    ...
    for i in 0..3 {
        assert!(matches!(entries[i].phase, LogPhase::Build), "First 3 should be build phase");
    }
    for i in 3..5 {
        assert!(matches!(entries[i].phase, LogPhase::Runtime), "Last 2 should be runtime phase");
    }
