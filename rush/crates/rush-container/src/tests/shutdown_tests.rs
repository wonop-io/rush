...
async fn test_exit_codes_ignored_during_shutdown() -> Result<()> {
    ...
    for code in ignored_codes {
        ...
        assert!(is_ignored, "Exit code {} should be ignored during shutdown", code);
    }
...
async fn test_capture_with_shutdown_handles_cancellation() -> Result<()> {
    ...
    assert!(shutdown_token.is_cancelled(), "Shutdown should be triggered");
...
async fn test_shutdown_cleanup_sequence() -> Result<()> {
    ...
    let container_ids = vec![
        docker_client.run_container("app:1", "app1", "net", &[], &[], &[]).await?,
        docker_client.run_container("app:2", "app2", "net", &[], &[], &[]).await?,
    ];
    ...
    let stop_indices: Vec<_> = history.iter()
        .enumerate()
        .filter(|(_, call)| call.starts_with("stop_container"))
        .map(|(i, _)| i)
        .collect();
    
    let remove_indices: Vec<_> = history.iter()
        .enumerate()
    ...
    if !stop_indices.is_empty() && !remove_indices.is_empty() {
        ...
        assert!(last_stop < first_remove, "All containers should be stopped before any are removed");
    }
...
async fn test_shutdown_reason_propagation() -> Result<()> {
    ...
    for reason in reasons {
        ...
        match receiver.try_recv() {
            Ok(received) => {
                match (expected_reason, received) {
                    (ShutdownReason::UserRequested, ShutdownReason::UserRequested) => {},
                    (ShutdownReason::ContainerExit, ShutdownReason::ContainerExit) => {},
                    (ShutdownReason::Completed, ShutdownReason::Completed) => {},
...
async fn test_container_exit_triggers_shutdown() -> Result<()> {
    ...
    docker_client.set_container_status(&container_id, ContainerStatus::Exited(1)).await;
    ...
    assert!(matches!(status, ContainerStatus::Exited(_)), "Container should be exited");
