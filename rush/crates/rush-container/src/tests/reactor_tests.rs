...
async fn test_container_crash_simulation() -> Result<()> {
    ...
    assert_eq!(status, ContainerStatus::Exited(1), "Container should show as crashed");
...
async fn test_build_failure_simulation() -> Result<()> {
    ...
    // Attempt build
    let result = docker_client.build_image("test:1", "Dockerfile", "/context").await;
...
async fn test_container_monitoring() -> Result<()> {
    ...
    // Simulate exit
    docker_client.set_container_status(&container_id, ContainerStatus::Exited(0)).await;
...
async fn test_build_failure_waits_for_changes() -> Result<()> {
    ...
    assert!(should_wait_for_file_changes, "Build failures should wait for file changes");
    assert!(!should_retry_on_timeout, "Build should NOT retry on timeout, only on file changes");
    assert!(should_exit_on_termination, "Build should exit when terminated by user");
