...
use crate::{
    docker::{
        DockerClient, DockerClientWrapper, DockerWrapperConfig,
        ConnectionPool, PoolConfig, PooledDockerClient,
        LogStreamManager, LogStreamConfig,
...
impl DockerIntegration {
    ...
    pub fn new(
        ...
        let client = Self::create_client_chain(
            base_client,
            &config,
            event_bus.clone(),
        )?;
    ...
    fn create_client_chain(
        ...
        if config.use_enhanced_client {
            ...
            let mut wrapper = DockerClientWrapper::new(
                client,
                config.wrapper_config.clone(),
            );
    ...
    pub async fn start_log_streaming(&self, container_id: String, component: String) -> Result<()> {
        if let Some(manager) = &self.log_manager {
            let receiver = manager.start_streaming(container_id.clone(), component.clone()).await;
            ...
            tokio::spawn(async move {
                ...
                while let Some(entry) = receiver.recv().await {
                    debug!("[{}] {}: {}", entry.component, entry.level as u8, entry.message);
                    
                    // Publish important log events
                    if entry.level >= crate::docker::LogLevel::Error {
                        let _ = event_bus.publish(crate::events::Event::error(
                            &entry.component,
                            entry.message.clone(),
                            false,
                        )).await;
    ...
    pub async fn start_operation(&self, op_type: OperationType) -> Option<crate::docker::metrics::OperationTimer> {
    ...
    pub async fn run_container(
        ...
        if let Ok(ref container_id) = result {
            let _ = self.start_log_streaming(
                container_id.clone(),
                name.to_string(),
            ).await;
        }
...
mod tests {
    ...
    fn test_docker_integration_builder() {
        let builder = DockerIntegrationBuilder::new()
            .with_config(DockerIntegrationConfig::default());
