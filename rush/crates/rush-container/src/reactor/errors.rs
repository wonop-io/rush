...
pub enum ReactorError {
    ...
    Build { component: String, error: String },

    /// Container startup failed
    #[error("Failed to start container {container}: {error}")]
    ContainerStart { container: String, error: String },

    /// Container health check failed
    #[error("Health check failed for {container}: {reason}")]
    HealthCheck { container: String, reason: String },
...
mod tests {
    ...
    fn test_error_creation() {
        ...
        assert_eq!(
            err.to_string(),
            "Docker operation failed: Failed to connect"
        );

        let err = ReactorError::build("frontend", "webpack failed");
        assert_eq!(
            err.to_string(),
            "Build failed for component frontend: webpack failed"
        );
