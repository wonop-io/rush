...
impl ConfigRepository {
        ...
        T: for<'de> Deserialize<'de>,
    {
        let value = self.get(key).await
            .ok_or_else(|| Error::Internal(format!("Configuration key '{}' not found", key)))?;
    ...
    pub async fn remove(&self, key: &str) -> Result<()> {
        ...
        if let Some(entry) = entries.remove(key) {
            ...
            let watchers = self.watchers.read().await;
            for watcher in watchers.iter() {
                watcher.on_change(ConfigChange::Removed(key.to_string()), Some(&entry)).await;
            }
        }
        Ok(())
    }
    ...
    pub async fn merge(&self, other: &ConfigRepository) -> Result<()> {
        ...
        for (key, entry) in other_entries.iter() {
            self.set_with_source(key.clone(), entry.value.clone(), entry.source.clone()).await?;
        }
        Ok(())
    }
...
impl ConfigValidator for SchemaValidator {
    fn validate(&self, entry: &ConfigEntry) -> Result<()> {
        ...
        for (pattern, _schema) in &self.schemas {
...
mod tests {
    ...
    async fn test_basic_operations() {
        ...
        // Set a value
        repo.set("test.key", JsonValue::String("value".to_string())).await.unwrap();
        ...
        // Update the value
        repo.set("test.key", JsonValue::String("new_value".to_string())).await.unwrap();
        let value = repo.get("test.key").await;
    ...
    async fn test_list_keys() {
        ...
        repo.set("key1", JsonValue::String("value1".to_string())).await.unwrap();
        repo.set("key2", JsonValue::String("value2".to_string())).await.unwrap();
        repo.set("key3", JsonValue::String("value3".to_string())).await.unwrap();
    ...
    async fn test_export_import() {
        ...
        repo1.set("test.a", JsonValue::String("a".to_string())).await.unwrap();
        repo1.set("test.b", JsonValue::String("b".to_string())).await.unwrap();
        ...
        assert_eq!(repo2.get("test.a").await, Some(JsonValue::String("a".to_string())));
        assert_eq!(repo2.get("test.b").await, Some(JsonValue::String("b".to_string())));
    }
}