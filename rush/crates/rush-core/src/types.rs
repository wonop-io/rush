...
impl TargetArchitecture {
    ...
    ///
    /// Always returns a Linux platform because Docker containers run Linux.
    /// For native architecture, returns the Linux platform matching the host CPU.
    pub fn to_bazel_platform(&self) -> Option<String> {
        match self {
            TargetArchitecture::Native => {
                // Docker containers always run Linux, so we need a Linux platform
                // even when building "natively" on macOS
                let arch = std::env::consts::ARCH;
                match arch {
                    "x86_64" => Some("//platforms:linux_amd64".to_string()),
                    "aarch64" => Some("//platforms:linux_arm64".to_string()),
                    _ => None, // Unknown architecture, let Bazel decide
                }
            }
