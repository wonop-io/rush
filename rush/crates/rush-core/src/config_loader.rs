...
impl ConfigLoader {
    ...
    pub fn load_yaml<T: DeserializeOwned>(path: &Path) -> Result<T> {
        ...
        serde_yaml::from_str(&content)
            .map_err(|e| Error::Config(format!("Failed to parse YAML from {}: {}", path.display(), e)))
    }
    ...
    pub fn load_json<T: DeserializeOwned>(path: &Path) -> Result<T> {
        ...
        serde_json::from_str(&content)
            .map_err(|e| Error::Config(format!("Failed to parse JSON from {}: {}", path.display(), e)))
    }
    ...
    pub fn load_toml<T: DeserializeOwned>(path: &Path) -> Result<T> {
        ...
        toml::from_str(&content)
            .map_err(|e| Error::Config(format!("Failed to parse TOML from {}: {}", path.display(), e)))
    }
    ...
    pub fn load_auto<T: DeserializeOwned>(path: &Path) -> Result<T> {
        ...
        match extension.to_lowercase().as_str() {
            ...
            _ => Err(Error::Config(format!("Unsupported file extension: {}", extension))),
        }
    }
