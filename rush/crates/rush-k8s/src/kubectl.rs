...
impl Kubectl {
    ...
    pub async fn apply(&self, manifest_path: &Path) -> Result<KubectlResult> {
        let mut args = vec!["apply".to_string(), "-f".to_string(), manifest_path.display().to_string()];
    ...
    pub async fn apply_dir(&self, dir_path: &Path) -> Result<Vec<KubectlResult>> {
        if !dir_path.is_dir() {
            return Err(Error::Filesystem(format!("{} is not a directory", dir_path.display())));
        }
            ...
            for entry in entries {
                ...
                if path.is_file() {
                    if let Some(name) = path.file_name() {
                        if name.to_string_lossy().ends_with(&format!("-{}.yaml", kind)) ||
                            ...
                            match self.apply(&path).await {
                                Ok(result) => {
                                    if result.success {
                                        ...
                                        warn!("Failed to apply {}: {}", path.display(), result.stderr);
                                    }
                                    results.push(result);
                                }
                                Err(e) => {
                            ...
                            match self.delete(&path).await {
                                Ok(result) => {
                                    if result.success {
                                        ...
                                        warn!("Issue deleting {}: {}", path.display(), result.stderr);
                                    }
                                    results.push(result);
                                }
                                Err(e) => {
    ...
    pub async fn execute(&self, mut args: Vec<String>) -> Result<KubectlResult> {
        ...
        if self.config.verbose {
            debug!("Executing kubectl command: {} {}", self.config.kubectl_path, args.join(" "));
        }
