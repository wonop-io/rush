...
impl ManifestGenerator {
    ...
    pub async fn generate_manifests(
        ...
        info!("Generating Kubernetes manifests for {} components", components.len());
        ...
        for component in components {
            ...
            if let Some(service) = self.generate_service(component)? {
                let path = self.output_dir.join(format!("{}-service.yaml", component.component_name));
    ...
    fn generate_deployment(&self, component: &ComponentBuildSpec) -> Result<Option<GeneratedManifest>> {
        ...
        metadata.insert("name".to_string(), serde_yaml::Value::String(component.component_name.clone()));
        metadata.insert("namespace".to_string(), serde_yaml::Value::String(self.namespace.clone()));
        
        let mut labels = BTreeMap::new();
        labels.insert("app".to_string(), serde_yaml::Value::String(component.component_name.clone()));
        labels.insert("env".to_string(), serde_yaml::Value::String(self.environment.clone()));
        metadata.insert("labels".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(labels)));
        
        deployment.insert("metadata".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(metadata)));
        
        // Spec
        let mut spec = BTreeMap::new();
        spec.insert("replicas".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(1)));
        
        // Selector
        let mut selector = BTreeMap::new();
        let mut match_labels = BTreeMap::new();
        match_labels.insert("app".to_string(), serde_yaml::Value::String(component.component_name.clone()));
        selector.insert("matchLabels".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(match_labels)));
        spec.insert("selector".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(selector)));
        ...
        template_labels.insert("app".to_string(), serde_yaml::Value::String(component.component_name.clone()));
        template_metadata.insert("labels".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(template_labels)));
        template.insert("metadata".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(template_metadata)));
        ...
        container.insert("name".to_string(), serde_yaml::Value::String(component.component_name.clone()));
        
        // Determine image
        let image = self.get_image_name(component)?;
        container.insert("image".to_string(), serde_yaml::Value::String(image));
        container.insert("imagePullPolicy".to_string(), serde_yaml::Value::String("Always".to_string()));
        
        // Ports if specified
        if let Some(port) = component.port {
            let mut port_spec = BTreeMap::new();
            port_spec.insert("containerPort".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(port)));
            
            let ports = vec![serde_yaml::Value::Mapping(Self::btree_to_mapping(port_spec))];
        ...
        env_var.insert("name".to_string(), serde_yaml::Value::String("ENVIRONMENT".to_string()));
        env_var.insert("value".to_string(), serde_yaml::Value::String(self.environment.clone()));
        ...
        if let Some(env) = &component.env {
            for (key, value) in env {
                ...
                env_var.insert("value".to_string(), serde_yaml::Value::String(value.clone()));
                env_vars.push(serde_yaml::Value::Mapping(Self::btree_to_mapping(env_var)));
            }
        }
        ...
        spec.insert("template".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(template)));
        
        deployment.insert("spec".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(spec)));
    ...
    fn generate_service(&self, component: &ComponentBuildSpec) -> Result<Option<GeneratedManifest>> {
        ...
        service.insert("apiVersion".to_string(), serde_yaml::Value::String("v1".to_string()));
        service.insert("kind".to_string(), serde_yaml::Value::String("Service".to_string()));
        ...
        service.insert("metadata".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(metadata)));
        
        // Spec
        let mut spec = BTreeMap::new();
        spec.insert("type".to_string(), serde_yaml::Value::String("ClusterIP".to_string()));
        
        // Selector
        let mut selector = BTreeMap::new();
        selector.insert("app".to_string(), serde_yaml::Value::String(component.component_name.clone()));
        spec.insert("selector".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(selector)));
        
        // Ports
        let mut port_spec = BTreeMap::new();
        port_spec.insert("port".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(port)));
        port_spec.insert("targetPort".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(port)));
        port_spec.insert("protocol".to_string(), serde_yaml::Value::String("TCP".to_string()));
        
        spec.insert("ports".to_string(), 
            serde_yaml::Value::Sequence(vec![serde_yaml::Value::Mapping(Self::btree_to_mapping(port_spec))]));
        
        service.insert("spec".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(spec)));
    ...
    fn generate_ingress(&self, components: &[ComponentBuildSpec]) -> Result<Option<GeneratedManifest>> {
        // Filter components that have mount points (exposed via ingress)
        let exposed_components: Vec<_> = components.iter()
        ...
        ingress.insert("apiVersion".to_string(), serde_yaml::Value::String("networking.k8s.io/v1".to_string()));
        ingress.insert("kind".to_string(), serde_yaml::Value::String("Ingress".to_string()));
        
        // Metadata
        let mut metadata = BTreeMap::new();
        metadata.insert("name".to_string(), serde_yaml::Value::String(format!("{}-ingress", self.environment)));
        metadata.insert("namespace".to_string(), serde_yaml::Value::String(self.namespace.clone()));
        
        let mut annotations = BTreeMap::new();
        annotations.insert("kubernetes.io/ingress.class".to_string(), 
            serde_yaml::Value::String("nginx".to_string()));
        metadata.insert("annotations".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(annotations)));
        
        ingress.insert("metadata".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(metadata)));
        ...
        for component in exposed_components {
            if let Some(mount_point) = &component.mount_point {
                ...
                path_item.insert("path".to_string(), serde_yaml::Value::String(mount_point.clone()));
                path_item.insert("pathType".to_string(), serde_yaml::Value::String("Prefix".to_string()));
                
                let mut backend = BTreeMap::new();
                let mut service = BTreeMap::new();
                service.insert("name".to_string(), serde_yaml::Value::String(component.component_name.clone()));
                
                let mut port = BTreeMap::new();
                port.insert("number".to_string(), 
                    serde_yaml::Value::Number(serde_yaml::Number::from(component.port.unwrap_or(8080))));
                service.insert("port".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(port)));
                
                backend.insert("service".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(service)));
                
                path_item.insert("backend".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(backend)));
                
                paths.push(serde_yaml::Value::Mapping(Self::btree_to_mapping(path_item)));
            }
        }
        
        http.insert("paths".to_string(), serde_yaml::Value::Sequence(paths));
        rule.insert("http".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(http)));
        
        spec.insert("rules".to_string(), serde_yaml::Value::Sequence(vec![serde_yaml::Value::Mapping(Self::btree_to_mapping(rule))]));
        
        ingress.insert("spec".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(spec)));
    ...
    pub fn generate_configmap(&self, name: String, data: BTreeMap<String, String>) -> Result<GeneratedManifest> {
        let mut configmap = BTreeMap::new();
        configmap.insert("apiVersion".to_string(), serde_yaml::Value::String("v1".to_string()));
        configmap.insert("kind".to_string(), serde_yaml::Value::String("ConfigMap".to_string()));
        ...
        configmap.insert("metadata".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(metadata)));
        ...
        configmap.insert("data".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(data_map)));
    ...
    fn generate_secret(&self, secrets: BTreeMap<String, String>) -> Result<Option<GeneratedManifest>> {
        if secrets.is_empty() {
            return Ok(None);
        }
        
        let mut secret = BTreeMap::new();
        secret.insert("apiVersion".to_string(), serde_yaml::Value::String("v1".to_string()));
        secret.insert("kind".to_string(), serde_yaml::Value::String("Secret".to_string()));
        
        // Metadata
        let mut metadata = BTreeMap::new();
        metadata.insert("name".to_string(), serde_yaml::Value::String(format!("{}-secrets", self.environment)));
        metadata.insert("namespace".to_string(), serde_yaml::Value::String(self.namespace.clone()));
        
        secret.insert("metadata".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(metadata)));
        
        secret.insert("type".to_string(), serde_yaml::Value::String("Opaque".to_string()));
        ...
        secret.insert("data".to_string(), serde_yaml::Value::Mapping(Self::btree_to_mapping(data)));
