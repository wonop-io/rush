...
impl MultiClusterManager {
    ...
    pub fn set_current_cluster(&mut self, name: String) -> Result<()> {
        if self.get_cluster(&name).is_none() {
            return Err(Error::Configuration(format!("Cluster '{name}' not found")));
        }
    ...
    pub async fn deploy_to_clusters(
        ...
        match deployment_strategy {
            ...
            MultiClusterStrategy::BlueGreen => {
                ...
                let inactive_clusters: Vec<_> =
                    self.clusters.iter().filter(|c| !c.is_primary).collect();
        ...
        if !check_output.status.success() {
            ...
            if !create_output.status.success() {
                ...
                return Err(Error::Command(format!(
                    "Failed to create namespace: {stderr}"
                )));
            }
        }
...
"#,
            ...
            namespace,
            quota.cpu_request,
            quota.memory_request_gb,
            quota.cpu_limit,
            quota.memory_limit_gb,
            quota.pvc_count,
            quota.nodeport_count
        );
        ...
        if !output.status.success() {
            ...
            return Err(Error::Command(format!(
                "Failed to apply resource quota: {stderr}"
            )));
        }
