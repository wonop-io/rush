...
pub async fn create_context(
    ...
    // Create toolchain
    let toolchain = create_toolchain(&target_os, &target_arch, &toolchain_mode)?;
...
fn get_target_arch(matches: &ArgMatches) -> String {
    if let Some(target_arch) = matches.get_one::<String>("target_arch") {
        let arch = match target_arch.as_str() {
            ...
            other => {
                warn!("Unknown architecture '{other}', using native host architecture");
        ...
        let default_arch = std::env::consts::ARCH.to_string();
        info!("Target architecture: {default_arch} (native)");
        default_arch
    }
}
...
fn get_toolchain(matches: &ArgMatches, target_arch: &str) -> String {
    if let Some(toolchain) = matches.get_one::<String>("toolchain") {
        ...
        // If target_arch is the same as host, this means "native"
        let host_arch = std::env::consts::ARCH;
        if target_arch == host_arch {
            info!("Toolchain mode: native (host architecture matches target)");
            "native".to_string()
        } else {
            info!("Toolchain mode: {target_arch} (cross-compilation)");
            target_arch.to_string()
        }
    }
}
